...
class ssr2osr:
        ...
        for bb, low_rate in enumerate(ssr.lr):
            for sg, lr_msg in enumerate(low_rate):
                for kk, lr_sv_array in enumerate(lr_msg.sv_array):
                    ...
                    gnss = sat_list[0][0]
                    ## edit
                    #signals for the different satellites
                    ...
                    for jj, sat in enumerate(sat_list):
                        ...
                        # ####################################
                        # loop over signals, edit
                        # ####################################
                        sig_list = signals.get(sat[0])

                        for gg, signal in enumerate(sig_list):
                            #append signal component per satellite
                            self.nav_clock[ii][jj].append([])
                            self.orb[ii][jj].append([])
                            self.clk[ii][jj].append([])
                            self.clk_hr[ii][jj].append([])
                            self.cbias[ii][jj].append([])
                            self.pbias[ii][jj].append([])
                            self.gvi[ii][jj].append([])
                            self.gsi[ii][jj].append([])
                            self.gri[ii][jj].append([])
                            self.rsi[ii][jj].append([])
                            self.gt[ii][jj].append([])
                            self.rt[ii][jj].append([])
                            self.grt[ii][jj].append([])
                            self.wup[ii][jj].append([])
                            self.shap[ii][jj].append([])
                            self.sat_state[ii][jj].append([])
                            self.el[ii][jj].append([])
                            self.az[ii][jj].append([])
                            self.gsi_ppo[ii][jj].append([])
                            self.rsi_ppo[ii][jj].append([])
                            ...
                            #self.iode[ii][jj][gg] = iode #not sure
                            #radial, along and cross corrections per satellite
                            dr = lr_msg.rad[ii][jj]
                            dt = lr_msg.atr[ii][jj]
                            dc = lr_msg.ctr[ii][jj]
                            ...
                            if ephemeris is None:
                                print('WARNING: no ephemeris data for ' + sat) #todo check
                            ...
                            if ((gnss == 'G') | (gnss == 'E') | (gnss == 'J')):
                                ...
                                ls = None  # e.g. for GLO it is computed from eph #todo needs to update to adjust for signals

                                # todo necesarry?
                                #if jj == len(sat_list) - 1:
                                #    ii += 1  # increase system index
                                #    print("system index increased")
                                #    continue
                            ...
                            self.sat_state[ii][jj][gg] = sat_state
                            self.nav_clock[ii][jj][gg] = sat_nav_clk
                            ...
                            # GPS/QZSS(L1), GLONASS(L1), Galileo(E1) and Beidou(2I) #todo signal freq update

                            if gnss == 'R': #Glonass sorted out first, as they have modular frequencies

                                if signal[0] == '2': #2C/2P signal
                                    ch = ephemeris.freq_num
                                    fr = 1246.000e6 + ch*716
                                elif signal[0] == '3': #G3
                                    fr = 1202.025e6
                                elif signal == '1C': #R1C signal
                                    ch = ephemeris.freq_num
                                    fr = (1602 + ch * 9.0 / 16.0) * 1e6

                            else:
                                if signal =='1C': #L1 C/A, B1C, EI c/x
                                    ch = 0
                                    fr = 2.0 * 77.0 * 10.23 * 1e6 #1575.42mhz, float reasons?
                                elif signal == '2I': #Beidou 2I (B1l)
                                    fr = 1561.098 * 1e6
                                elif signal =='2W': #L2 C/P
                                    fr= 1227.60e6
                                elif signal[0] =='5': #L5, E5(a), B2a
                                    fr= 1176.45e6
                                elif signal[0] =='8': #E5, BDSb2
                                    fr = 1191.795e6
                                elif signal[0] =='6': #B3, E6
                                    if gnss == 'C': # Beidou is 10 mhz lower, this allows for multiple signals
                                        fr = 1268.520e6
                                    else:
                                        fr = 1278.75e6
                                elif signal[0] =='7': #E5b, B2l, B2b
                                    fr= 1207.140e6
                            # wavelength
                            wavelength = trafo.Constants().c / fr  # [m]
                            # compute orbit and clock obs line corrections
                            if np.any(ssr.lr):
                                self.orb[ii][jj][gg] = OrbCorr(lr_msg, hr_msg,
                                                           sat_state,
                                                           self.rec, kk, kk_hr,
                                                           jj).corr
                                ...
                                clock_corr = ClockCorr(lr_msg, hr_msg, dt, kk,
                                                       kk_hr, jj)
                                self.clk[ii][jj][gg] = clock_corr.corr
                                self.clk_hr[ii][jj][gg] = clock_corr.corr_hr
                            else:
                                self.orb[ii][jj][gg] = np.nan
                                self.clk[ii][jj][gg] = np.nan
                            orb_out = self.make_output_format(self.orb[ii][jj][gg])
                            clk_out = self.make_output_format(self.clk[ii][jj][gg])
                            clk_hr_out = self.make_output_format(
                                self.clk_hr[ii][jj][gg])
                            clk_tot_out = self.make_output_format(self.clk[ii][jj][gg] +
                                                                  self.clk_hr[ii][jj][gg])

                            # code and phase bias
                            if np.any(ssr.lr): #todo check for which signal there is, and apply the correction for them

                                self.cbias[ii][jj][gg] = CodeBias(lr_msg, kk, jj, gnss,
                                                              signal).corr
                                self.pbias[ii][jj][gg] = PhaseBias(lr_msg, kk, jj,
                                                               gnss, signal).corr
                                # compute wind up effect
                                self.wup[ii][jj][gg] = WindUp(sat_state, fr, self.rec,
                                                          lat, lon).corr
                            else:
                                self.cbias[ii][jj][gg] = np.nan
                                self.pbias[ii][jj][gg] = np.nan
                                self.wup[ii][jj][gg] = np.nan
                            if self.csv_out:
                                cbias = -self.cbias[ii][jj][gg]
                                pbias = (-self.pbias[ii][jj][gg] * trafo.Constants().c
                                         / fr)  # [m]
                            else:
                                cbias = self.cbias[ii][jj][gg]
                                pbias = (self.pbias[ii][jj][gg] * trafo.Constants().c
                            ...
                            #######################################################
                            if self.csv_out:
                                wup = self.wup[ii][jj][gg] / wavelength
                            else:
                                wup = self.wup[ii][jj][gg]
                            wup_out = self.make_output_format(wup)

                            # compute relativistic shapiro effect
                            self.shap[ii][jj][gg] = ShapiroEffect(sat_state[0:3],
                                                              self.rec).corr
                            shap_out = self.make_output_format(self.shap[ii][jj][gg])
                            #######################################################
                            #                         Iono
                            #######################################################
                            iono_tot_tecu = 0
                            ...
                            if (np.any(ssr.gvi)):
                                ...
                                if gvi_msg.time_tag_15 >= lr_msg.time_tag_15:
                                    ...
                                    self.gvi[ii][jj][gg] = glo_vtec.corr
                                    iono_tot_tecu += glo_vtec.corr
                                    iono_tot_meters += glo_vtec.corr_f1
                                else:
                                    self.gvi[ii][jj][gg] = np.nan
                            else:
                                self.gvi[ii][jj][gg] = np.nan
                            gvi_out = self.make_output_format(self.gvi[ii][jj][gg])
                            ...
                            if len(lr_msg.gsi_coeff) > 0:
                                ...
                                glo_stec = Ionosphere(lr_msg, epoch, kk, jj,
                                                      ...
                                                      receiver, gnss, sat,
                                                      fr, iono_type,)
                                self.gsi[ii][jj][gg] = glo_stec.corr
                                iono_tot_tecu += glo_stec.corr
                                iono_tot_meters += glo_stec.corr_f1
                            else:
                                self.gsi[ii][jj][gg] = np.nan
                            gsi_out = self.make_output_format(self.gsi[ii][jj][gg])
                            ...
                            if (np.any(ssr.rsi[bb][sg])):
                                ...
                                if rsi_msg.time_tag_15 >= lr_msg.time_tag_15:
                                    ...
                                    reg_iono = Ionosphere(
                                        ...
                                        fr=fr,
                                        iono_type=iono_type,
                                        rsi_ppo=self.rsi_ppo[ii][jj][gg]
                                    )
                                    self.rsi[ii][jj][gg] = reg_iono.corr
                                    self.rsi_ppo[ii][jj][gg] = reg_iono.rsi_ppo

                                    iono_tot_tecu += reg_iono.corr
                                    iono_tot_meters += reg_iono.corr_f1
                                else:
                                    self.rsi[ii][jj][gg] = np.nan
                            else:
                                self.rsi[ii][jj][gg] = np.nan
                            rsi_out = self.make_output_format(self.rsi[ii][jj][gg])
                            # ellipsoidal elevation
                            [self.az[ii][jj][gg],
                             self.el[ii][jj][gg]] = trafo.compute_az_el(sat_state[0:3],
                                                                    self.rec,
                                                                    np.deg2rad(
                                                                        lat),
                                                                    np.deg2rad(lon))
                                ...
                                if gri_msg is None:
                                    ...
                                    if gri_msg.time_tag_15 >= lr_msg.time_tag_15:
                                        ...
                                        grid_iono = Ionosphere(
                                            ...
                                            md=md,
                                            el=self.el[ii][jj][gg])
                                        self.gri[ii][jj][gg] = grid_iono.corr
                            ...
                            gri_out = self.make_output_format(self.gri[ii][jj][gg])
                                ...
                                for cc, comp in enumerate(ssr_rt.components):
                                    ...
                                    if ssr_rt.time_tag_15 >= lr_msg.time_tag_15:
                                        ...
                                        if comp == "m":
                                            ...
                                            self.rt[ii][jj][gg] += Troposphere(epoch, ssr_rt,
                                                                           receiver['ellipsoidal'],
                                                                           self.el[ii][jj][gg],
                                                                           doy,
                                                                           comp, cc,
                                            ...
                                            tropo_tot_corr_meters += self.rt[ii][jj][gg]
                                    else:
                                        self.rt[ii][jj][gg] = np.nan
                            else:
                                self.rt[ii][jj][gg] = np.nan
                            rt_out = self.make_output_format(self.rt[ii][jj][gg])
                            # gridded troposphere
                            self.grt[ii][jj][gg] = .0
                            ...
                            if (np.any(ssr_grt)):
                                for cc, comp in enumerate(ssr_grt.components):
                                    tropo_type = 'grt'
                                    self.grt[ii][jj][gg] += Troposphere(epoch, ssr_grt,
                                                                    receiver['ellipsoidal'],
                                                                    self.el[ii][jj][gg],
                                                                    doy,
                                                                    comp, cc,
                                    ...
                                    tropo_tot_corr_meters += self.grt[ii][jj][gg]
                            else:
                                self.grt[ii][jj][gg] = np.nan
                            grt_out = self.make_output_format(self.grt[ii][jj][gg])
                            ...
                            tropo_model = tropo_computation.TropoComputation(
                                epoch, None, receiver['ellipsoidal'], self.el[ii][jj][gg],
                            ...
                            el_deg = np.rad2deg(self.el[ii][jj][gg])
                            az_deg = np.rad2deg(self.az[ii][jj][gg])
                            el_out = self.make_output_format(el_deg)
                            az_out = self.make_output_format(az_deg)
                            self.epoch = epoch
                            iode_out=self.make_output_format(iode)
                            dr_out=self.make_output_format(dr)
                            dt_out=self.make_output_format(dt)
                            dc_out=self.make_output_format(dc)

                            # print output only for satellites with
                            # elevation larger than 0
                            if self.el[ii][jj][gg] >= 0:
                                if csv_out:
    ...
    def get_header_csv(cls):
        return ("".join(['# Week,time,SAT,signal,wavelength(m),azimuth(deg),',
                ...
                'solidEarthTide dU(m), ',
                'mappingImprovement(m), ','iode,','radial,','along,','cross']))
